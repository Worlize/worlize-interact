...
var PresenceUser = function(guid, userName) {
    ...
    this.startTimeForDurationRecording = null;
};
...
PresenceUser.prototype.initializeDurationRecording = function() {
    this.setStartTimeForDurationRecording();
    // Update stats DB every five minutes.
    this.updateDurationInterval = setInterval(this.handleUpdateDurationInterval.bind(this), 300000);  
};

PresenceUser.prototype.setStartTimeForDurationRecording = function() {
    this.startTimeForDurationRecording = (new Date()).valueOf();
};

PresenceUser.prototype.handleUpdateDurationInterval = function() {
    this.updateDuration();
};

PresenceUser.prototype.updateDuration = function() {
    var start = this.startTimeForDurationRecording;
    if (start === null) { return; }
    var duration = Math.round(((new Date()).valueOf() - start) / 1000);
    this.setStartTimeForDurationRecording();
    var statsDB = redisConnectionManager.getClient('stats');
    statsDB.zincrby('userTime', duration, this.guid);
};
...
PresenceUser.prototype.destroy = function() {
    ...
    this.unsubscribeFromPubSubChannel();
    if (this.presenceStatusUpdateInterval) {
        clearInterval(this.presenceStatusUpdateInterval);
    }
};
