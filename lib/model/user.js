var events = require('events'), 
    ...
    UserRestrictionsChangedMessage = require('../rpc/chatserver_messages/user_restrictions_changed'),
...
var User = function(guid, userName) {
    ...
    this._restrictionsChanged = this.handleRestrictionsChanged.bind(this);
};
...
User.prototype.afterRemovedFromRoom = function() {
    this.stopMonitoringRestrictionChanges();
};
...
User.prototype.applyRestrictions = function() {
    ...
    Banlist.isIPBanned(this.connection.remoteAddress, function(err, result) {
        if (err) {
...
User.prototype.monitorRestrictionChanges = function() {
    this.restrictions.on('changed', this._restrictionsChanged);
};

User.prototype.stopMonitoringRestrictionChanges = function() {
    this.restrictions.clearRefreshTimer();
    this.restrictions.removeListener('changed', this._restrictionsChanged);
};

User.prototype.handleRestrictionsChanged = function() {
    if (this.currentRoom) {
        var message = new UserRestrictionsChangedMessage();
        message.user = this;
        message.room = this.currentRoom;
