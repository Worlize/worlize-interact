...
var Room = function(chatserver, guid) {
    ...
    this.on('ready', function() {
        logger.info("Room " + self.guid + " ready. Name: " + self.name);
    });
};
...
Room.prototype.userEnter = function(newUser) {
    ...
    if (!this.usersByUserGuid[newUser.guid]) {
        logger.debug("Adding user " + newUser.guid + " to room " + this.guid);
        ...
        logger.debug("User already exists in room.  Not sending user_enter message.");
    }
};
