...
Room.prototype.addUser = function(user) {
    ...
    // Collect all the messages to send to the user so we can
    // blast them out all at once.
    var messageQueue = [];
    ...
    roomDefinitionMessage.room = this;
    messageQueue.push(roomDefinitionMessage);
    ...
    var apps = this.definition.items.filter(function(item) { return item.type === 'app'; });
    for (var i=0,len=apps.length; i < len; i ++) {
        var app = apps[i];
        var syncedListDumpMessage = new SyncedListDumpMessage();
        syncedListDumpMessage.appGuid = app.guid;
        syncedListDumpMessage.stateHistoryList = this.appStateHistoryLists[app.guid];
        messageQueue.push(syncedListDumpMessage);
        ...
        syncedDataDumpMessage.syncedDataStore = this.appSyncedDataStores[app.guid];
        messageQueue.push(syncedDataDumpMessage);
    ...
    messageQueue.push(videoServerMessage);
    ...
    for (var i=0,len=this.users.length; i < len; i ++) {
        ...
        messageQueue.push(userEnterMessage);
    ...
    user.loadPermissions(this.definition.worldGuid, function(err, permissions) {
        if (err) {
            logger.error("Disconnecting user: Unable to load user permissions: " + err, user.logNotation);
            failUserEntry();
            return;
        }
        continueAfterLoadingPermissions();
    });
    
    function continueAfterLoadingPermissions() {
        ...
        // Finally, send all the messages we have queued up
...
Room.prototype.mayUserEnter = function(guid) {
    ...
    if (this.locked && guid !== this.definition.ownerGuid) {
        return false;
    }
    return true;
};
    ...
    switch (decodedMessage.msg) {
        ...
        case "update_room_property":
            if (decodedMessage.data.name) {
                this.definition.properties[decodedMessage.data.name] = decodedMessage.data.value;
            }
            this.forwardMessageToUsers(decodedMessage);
            break;
