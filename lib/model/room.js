...
Room.prototype.addUser = function(user) {
    ...
    logger.debug("Adding user " + user.guid + " to room " + this.guid);
    ...
    if (oldUser) {
        logger.error("addUser: User already exists in room!  Disconnecting previous user.");
        oldUser.connection.close();
    }
    ...
    multi.exec(function(err, result) {
        if (err) {
            ...
            logger.error("Disconnecting user: Redis error while updating room occupancy data: " + err);
            setTimeout(function() {
...
Room.prototype.removeUser = function(user) {
    if (this.hasUser(user)) {
        ...
        multi.exec(function(err, results) {
            if (err) {
                // Something went horribly wrong with redis!
                logger.error("Redis error while updating room occupancy data: " + err);
            }
        });
...
Room.prototype.handlePubSubMessage = function(message) {
    ...
    try {
        ...
        switch (message.msg) {
            ...
            case "object_removed":
                this.load(function(err, data) {
                    ...
                    self.forwardMessageToUsers(message);
                });
                break;
            default:
        ...
        logger.error("Error while distributing message to room from control channel: " + e);
    }
};
