...
Room.prototype.initAppData = function() {
    var self = this;
    ...
    this.definition.items.forEach(function(item) {
        if (item.type === 'app') {
            if (!self.appStateHistoryLists[item.guid]) {
                self.appStateHistoryLists[item.guid] = new StateHistoryList(item.guid);
            }
        }
    });
    ...
    this.definition.items.forEach(function(item) {
        if (item.type === 'app') {
            if (!self.appSyncedDataStores[item.guid]) {
                self.appSyncedDataStores[item.guid] = {};
            }
        }
    })
};
...
Room.prototype.handlePubSubMessage = function(message) {
    ...
    switch (decodedMessage.msg) {
        ...
        case "remove_item":
            this.definition.removeItem(decodedMessage.data.item_guid);
            this.forwardMessageToUsers(decodedMessage);
            break;
