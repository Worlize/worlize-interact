...
PubSubManager.prototype.handleSubscribeConnectionReady = function() {
    ...
    if (subscribedChannels.length > 0) {
        logger.warn("Reconnected to redis pubsub server, re-establishing subscriptions for " + subscribedChannels.length + " previously listened channels");
        if (logger.shouldLogLevel('debug3')) {
            logger.debug3("Subscribed Channels: " + subscribedChannels.join(','));
        }
