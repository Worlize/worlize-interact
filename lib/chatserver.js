...
// Public methods
    ...
    setupHeartbeat: function() {
        ...
        var cmd = spawn('hostname');
        var hostname = "";
        cmd.stdout.addListener('data', function(data) {
            hostname = data;
        });
        cmd.addListener('exit', function(code) {
            this.hostname = hostname.toString().trim();
            if (code !== 0) {
                sys.puts("Unable to determine hostname.  You must explicitly provide a hostname");
                process.exit(1);
            }
            this._updatePresenceDatabase();
            setInterval(this._updatePresenceDatabase.bind(this), serverConfig.updatePresenceInterval);
        }.bind(this));
    },
...
// Private methods
    ...
    _updatePresenceDatabase: function() {
        ...
        redis.hset("interactServer", this.serverId, JSON.stringify(obj));
    }
});
