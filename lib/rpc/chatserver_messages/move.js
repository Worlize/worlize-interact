...
MoveMessage.prototype.receive = function(message, user) {
    ...
    if (message.data) {
        ...
        if (isNaN(x) || isNaN(y)) {
            ...
            logger.warn("User " + user.guid + " attempted to set non-numeric position.");
            this.send(user.connection);
            return;
        }
        if (x < 0 || y < 0 || x > config.roomWidth || y > config.roomHeight) {
            // Illegal position, reset user's position on client
            logger.warn("User " + user.guid + " attempted to set an out-of-bounds position.");
            this.send(user.connection);
            return;
        }
...
MoveMessage.prototype.send = function(connection) {
