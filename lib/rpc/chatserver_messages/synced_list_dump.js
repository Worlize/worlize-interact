...
Msg.prototype.send = function(user) {
    if (!this.buffer) {
        // msgID + appGuid + stateCount(32bit) + n*userGuid + totalSerializedMessageBytes
        var msgLength = 4 + 16 + 4 + (this.stateHistoryList.length * 20) + this.stateHistoryList.bytesUsed;
        ...
        this.stateHistoryList.forEach(function(item) {
            GUIDUtil.writeBytes(item.user, buffer, offset);
            offset += 16;
            
            buffer.writeUInt32BE(item.buffer.length, offset);
            offset += 4;
