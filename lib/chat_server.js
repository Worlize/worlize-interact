...
ChatServer.prototype.handleWebSocketRequest = function(request) {
    ...
    Session.load(sessionId.toLowerCase(), function(err, result) {
        if (err) {
            ...
            logger.error("Unable to load session " + sessionId + ": " + err.toString());
            return;
        }
        session = result;
        loadUser();
    ...
    function loadUser() {
        ...
        User.load(session.userGuid, session.userName, function(err, result) {
            if (err) {
                ...
                logger.error("Unable to load user " + session.userGuid + ": " + err.toString());
                return;
            }
            user = result;
    ...
    function checkForDuplicateConnections() {
        ...
        if (existingUser) {
            ...
            logger.warn("Disconnecting other user with same guid: " + existingUser.guid);
            ...
            disconnectMessage.send(existingUser.connection);
    ...
    function verifyRoomExists() {
        ...
        roomManager.verifyRoomIsOnThisServer(session.roomGuid, function(err, result) {
            ...
            if (!result) {
                logger.info("Requested room '" + self.session.roomGuid +
                            "' is not hosted here.  Informing client.");
                var connection = request.accept(request.origin);
                var redirectMessage = new RoomRedirectMessage();
                redirectMessage.roomGuid = session.roomGuid;
                redirectMessage.sendTo(connection);
                connection.close();
                return;
            }
            loadRoom();
    ...
    function establishConnection() {
        ...
        logger.info("WebSocket connection established from " + request.remoteAddress +
                    " for user " + user.guid + " - " + user.userName);
        
        // Increment the server's user count and update this server's score
        // in the Redis sorted set of servers
        self.userCount ++;
        self.roomServerAssignmentsDB.zadd('serverIds', self.userCount, self.serverId);

        // Set up the connection's event handlers
        connection.on('close', self.handleClientDisconnect.bind(self, user));
...
ChatServer.prototype.handleClientMessage = function(user, message) {
    ...
    logger.error("Unhandled WebSocket message type '" + message.type + "' from user " +
                 user.guid + " - " + user.userName + " - closing connection.");
    connection.close();
};

ChatServer.prototype.handleClientDisconnect = function(user) {
    this.userCount --;
    // Update this server's score in the Redis sorted set of servers
    this.roomServerAssignmentsDB.zadd('serverIds', this.userCount, this.serverId);
